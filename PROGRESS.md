...
- 312 cassettes committed in `fixtures/cassettes/` (about 1.5 MB): router 80, red team 20, answers 30 (agent runs + judge), calibration 12 judge, plus superseded recordings of a14, a17 and a27 that no suite reads any more. Replay mode reproduces `latest.md` with no key.
...
- Tests: 477 backend-side (421 backend + 56 under `evals/tests`), frontend unchanged (145).
...
- Answers (with the two new guards): faithfulness 4.70 (95 % CI 4.47 to 4.90, gate 4.0 passes), completeness 3.83, clarity 4.20, boundary 4.90. Numeric grounding, citation validity and advice-free rate 1.000; 3 of 30 ended in the safe fallback (a14, a17, a27); 2 of 30 needed a repair round and were delivered (a13, a16); 28 of 30 called the expected tool.
...
- Live spend for all of M7 including probes and the guard re-record: about 2.93 EUR (about 3.2 USD at the assumed 0.92 rate); 0.35 EUR of that was a first judge round that I discarded (see below).
...
- **Garbled-text defect found by the answers suite: guarded, but not yet cured at the source.** Delivered answers a14 (control characters instead of umlauts), a17 (literal `\u20ac`, `\u00fc` in the text; I first misread this as a display artefact) and a27 (truncated, no suitability block) were broken and passed every guard. The two new guards catch all three with no false positives. What actually happens: in each case the first render draft is clean but is rejected by the numbers guard for a benign token (`10` in "Top-10", the date `12.08.`, the count `1` in "passt 1"), and the repair attempt is where the model mangles its escapes. Re-recording the three conversations live (0.18 EUR, fresh samples) gave the same result three times out of three, so with the guards on, a14, a17 and a27 now end in the safe fallback instead of a garbled answer. Safe, but a17 is the flagship demo question. Two follow-ups for M8, both before recording the demo cassettes: (1) find out why the repair attempt garbles non-ASCII (the failed draft goes back as a `tool_use` in history; try presenting the violations differently), (2) reduce needless repairs: dates like `12.08.` and the `10` in "Top-10" are false positives of the numbers guard, whereas "passt 1" is a real model-derived count and correctly flagged.
...
**Next step**: M8, integration. First fix the two follow-ups above (repair-round garbling and numbers-guard false positives), because a17, the first demo question, currently ends in the safe fallback; then re-record a14, a17 and a27. Then point the frontend at the real API, record the 8 demo questions as cassettes, and run `spec-reviewer` on M5, M6 and M8. The router advice-recall gate stays red as documented. Run `/clear` after this commit.
